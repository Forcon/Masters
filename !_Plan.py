...
8. Добавления авторов в "черный список" из панели управления картинками и через управляющую панель
...
12. Сделать в базе поле "е-мейл" и "пароль" (или арес на ЯМ), чтобы отбирать только сбор определенного пользователя.
...
15. Убрать ошибку: Сделать чтобы поле для записи разблокировалось сразу после отказа от закрытия окна.

== 3й Приоритет ==
8. Сделать для"черной базы авторов". (Отдельная программа!) Должен быть режим просмотра этой базы + подтягивания работ этих авторов, с возможностью автора из базы исключить.
...
- Сделать программу по очистке основной базы данных от авторов, которые попали в "черный список" (если это единственный пользователь -- удаляем всю запись, иначе -- удаляем пользователя из колонки Coll_Autor)
--------
Не получается:
...
но не получается
- (7) записывать картинки тоже в базу, а не на диск
